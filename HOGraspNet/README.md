...
## Dawoon Kwon (Personal)
This codebase is built upon the following repositories:
* HOGraspNet: https://github.com/kaist-uvr-lab/HOGraspNet
* MANO layer (pth): https://github.com/hassony2/manopth
* CoMA (Reproduced for PyTorch): https://github.com/sw-gong/coma

### Environments

```bash
conda create -n kdw_grasp python=3.10
...
```bash
cd assets
# Google form 작성하고 발급받은 URL 그대로 넣으면 안 됨, 리디렉션 주기 때문에 그 URL로 넣어야 함
wget -O urls.zip "[URL]"
unzip urls.zip
cd ..
```
...
### Download data
```bash
# download_data.sh 내부 인자 수정 필요 (objModel 필요 없을 수도 있음)
# output_folder 인자 workspace/인지, data/인지 잘 확인할 것
sbatch scripts_/download_data.sh

# unzip_data_multi.py는 사용하지 않는 것이 좋음, data/ (HDD)에서 해서 그게 문제일 수도 있는데,
# 멀티 스레딩이나 멀티 프로세싱 방식이 문제가 아니었을 거라 확실하게 판단되지는 않음
# 기존 script인 unzip_data.py를 쓸 것
# 확실히 workspace/ (SSD)에서 하니까 속도 엄청 빨라짐
sbatch scripts_/unzip_data.sh

# base_path 인자 경로 내 폴더의 개수와 base_path 자체 용량 확인하는 스크립트
# --count 인자를 통해 폴더 개수 셀 것인지 말 것인지에 대한 유무 설정해줄 수 있음
sbatch scripts_/count_dir.sh
```

### Mesh Handling
```bash
conda activate kdw_grasp
# --no-index, --no-cache-dir 빼고 다운로드 해줘야 함. (넣으면 다운 안 됨)
pip install pytorch3d -f https://dl.fbaipublicfiles.com/pytorch3d/packaging/wheels/py310_cu118_pyt200/download.html

# MANO 사이트 가서 모델 pkl 받아와야 함
# 회원가입 하고, Download 들어가서 Models & Code 누르면 됨
# 서버에 올리고 압축해제 (경로는 thirdparty/)
7z x thirdparty/mano_v1_2.zip -o./thirdparty/

# HTML visualization (hand shape, pose, and contact map)
sbatch scripts_/mano_custom.sh
```